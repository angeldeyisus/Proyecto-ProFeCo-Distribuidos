...
export class PriceController {
  ...
  // GET /api/prices/wishlist-stats/:producto_id
  async obtenerEstadisticasWishlist(req, res) {
    try {
      const { producto_id } = req.params;

      // Buscar todas las preferencias que contengan el producto_id en su wishlist
      const interesados = await Preferencia.find({ wishlist: producto_id });

      const count = interesados.length;
      const emails = interesados.map(p => p.email).filter(Boolean); // Filtrar nulls/undefined
      ...
      console.error('Error obtenerEstadisticasWishlist:', error);
      res.status(500).json({ success: false, message: error.message });
    }
  }
